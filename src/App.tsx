  ...
  return <Shell>
    <main className="home">
      ...
      <section className="homeManifest">
        ...
        <article><i>02</i><h2>结果不止一个</h2><p>S 是本次和你匹配度最高的一组核心人格，A 是匹配度次高的共鸣人格，不代表稀有程度。</p></article>
...
function Reveal({ state, save }: { state: QuizState | null; save: (state: QuizState) => void }) {
  ...
  return <Shell quiet><main className={`revealPage ${gallery ? 'is-gallery' : 'is-ritual'}`}>
    ...
    {!gallery && current ? <section className="revealRitual">
      ...
      <div className="revealTitle"><span>PERSONALITY COLLECTION</span><h1>你的人格已经全部显现</h1><p>S 是本次和你匹配度最高的一组核心人格，A 是匹配度次高的共鸣人格，不代表稀有程度。</p></div>
